(function(){
  ...
  function getLines() {
    ...
    return raw.filter(l => l && typeof l.variantGid === "string" && Number(l.quantity) > 0);
  }

  function setLines(lines) {
    const filtered = (Array.isArray(lines) ? lines : []).filter(l => l && typeof l.variantGid === "string" && Number(l.quantity) > 0);
  ...
  function buildCartPermalink(lines) {
    ...
    const items = arr
      .filter(l => l && typeof l.variantGid === "string" && Number(l.quantity) > 0)
      .map(l => {
    ...
    const store = (window.ENV && (window.ENV.SHOPIFY_STOREFRONT_DOMAIN || window.ENV.SHOPIFY_STORE_DOMAIN)) || "7196su-vk.myshopify.com";
  ...
  window.CartPermalink = { gidToNumeric, getLines, setLines, buildCartPermalink };
})();
