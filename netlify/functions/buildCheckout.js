...
export const handler = async (event) => {
  ...
  try {
    ...
    if (data.userErrors && data.userErrors.length) {
      const msg = data.userErrors.map((e) => e.message).join(", ") || "Failed to create checkout";
      return createErrorResponse(msg, 400);
    }

    const cart = data.cart;
    if (!cart || !cart.checkoutUrl) return createErrorResponse("Checkout unavailable", 502);

    return createApiResponse({ checkoutUrl: cart.checkoutUrl, cartId: cart.id }, 200);
  } catch (err) {
