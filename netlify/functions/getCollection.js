...
export const handler = async (event, context) => {
  ...
  try {
    const { handle, tag, first = 20, after } = event.queryStringParameters || {};
    ...
    if (tag) {
      // Query products by tag within a collection
      query = `
        query GetCollectionWithTag($handle: String!, $tag: String!, $first: Int!, $after: String) {
